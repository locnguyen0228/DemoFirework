...
const store = {
  ...
  state: {
    ...
    // Note that config values used for s must be strings, unless manually converting values to strings
...
// ────────────────────────────────────────────────
// THÊM LOẠI PHÁO HOA HÌNH TRÁI TIM
// ────────────────────────────────────────────────
const heartShell = (size = 1) => {
  ...
  return {
    shellSize: size,
    spreadSize: 260 + size * 90, // hơi nhỏ hơn bình thường để trái tim gọn gàng
    starLife: 1100 + size * 220,
    starLifeVariation: 0.35,
    starDensity: 1.15,
    color,
    secondColor: useSecondColor,
    heart: true, // flag quan trọng để nhận diện
...
class Shell {
  ...
  burst(x, y) {
    ...
    if (this.heart) {
      // ──── HÌNH TRÁI TIM ────
      const heartPoints = 64; // số điểm để vẽ (càng nhiều càng mịn)
      const heartScale = this.spreadSize * 0.48;
      for (let i = 0; i < heartPoints; i++) {
        const t = (i / heartPoints) * PI_2;
        // Công thức parametric trái tim phổ biến
        ...
        const dist = Math.sqrt(tx * tx + ty * ty) / 16; // chuẩn hóa
        const speedMult = dist * 1.35; // điều chỉnh để trái tim đầy đặn hơn
        starFactory(angle, speedMult);
      }
      // Thêm một ít sao random bên trong để lấp đầy (tùy chọn)
      if (Math.random() < 0.6) {
        createBurst(Math.round(this.starCount * 0.25), starFactory);
      }
    } else if (this.ring) {
...
const soundManager = {
   ...
   * descreases volume and increase playback speed. This is because large explosions are
